function results = myRF_surrogate_LLRO(X_all2,Y2)
rng(1)
...
for j=1:length(ntree)
    AA_opt{j}=TreeBagger(ntree(j),train_int_norm,labtrain_int,'NumPredictorsToSample','all','Method','regression','PredictorSelection','curvature','Surrogate','on');
...
AA_ext=TreeBagger(ntree(optntree),train_ext_norm,labtrain_ext,'NumPredictorsToSample','all','Method','regression','OOBPredictorImportance','on','PredictorSelection','curvature','Surrogate','on');
