function results = myRF_median_LLRO(X_all2,Y2)
rng(1)
...
temp1=nanmedian(train_ext);
    for ll=1:numel(temp1)
        train_ext(isnan(train_ext(:,ll)),ll)=temp1(ll);
    end

test_ext=X_all2(idx_test,:);
    for ll=1:numel(temp1)
        test_ext(isnan(test_ext(:,ll)),ll)=temp1(ll);
    end
...
for j=1:length(ntree)
    AA_opt{j}=TreeBagger(ntree(j),train_int_norm,labtrain_int,'NumPredictorsToSample',82,'Method','regression');
...
AA_ext=TreeBagger(ntree(optntree),train_ext_norm,labtrain_ext,'NumPredictorsToSample',82,'Method','regression','OOBPredictorImportance','on');
