function results = myRF_surrogate_CVOS10(X_all2,Y2)
rng(1)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for h=1:in
        ...
        for j=1:length(ntree)
            AA_opt{h,j}=TreeBagger(ntree(j),train_int_norm,labtrain_int,'NumPredictorsToSample','all','Method','regression','PredictorSelection','curvature','Surrogate','on');
    ...
    %train time start
     tr_time=tic;
    AA_ext{i}=TreeBagger(ntree(optntree),train_ext_norm,labtrain_ext,'NumPredictorsToSample','all','Method','regression','OOBPredictorImportance','on','PredictorSelection','curvature','Surrogate','on');
